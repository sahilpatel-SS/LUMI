...
export function CircularGauge({ score, maxScore = 5, size = 140, color = '#2DD4A0' }: Props) {
  const r      = 44
  const cx     = 56
  const cy     = 56
  const C      = 2 * Math.PI * r
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-[2.2rem] font-light text-gray-900 leading-none select-none">
          {fmt(score)}
        </span>
      </div>

      <span className="absolute bottom-3 right-3 text-xs font-medium text-gray-400 select-none">
        /5
      </span>
    </div>
  )
